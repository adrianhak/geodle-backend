...
class GameRoundRetrieveAPIView(RetrieveAPIView):
    queryset = models.GameRound.objects
    serializer_class = serializers.GameRoundSerializer

    def get_object(self):
        instance = self.queryset.filter(date=datetime.date(datetime.now())).first()
        if instance is None:
            ...
            print(f'Instance found: ID: {instance.id}, Date: {instance.date}')
